...
import Data.List
import Distribution.Simple.Command (optArg')
...
oddFactorial 1 = 1
oddFactorial 2 = 1
oddFactorial x = if even x then (x-1) * oddFactorial (x-3)
...
leftpad str num = if length str >= num then str
                else leftpad (" " ++ str) num 
...
countdown x = "Ready! " ++ countdownHelper x "" ++ "Liftoff!"

countdownHelper :: Integer -> String -> String
countdownHelper 0 str = str
countdownHelper x str = countdownHelper (x - 1) (str ++ show x ++ "... ")
...
smallestDivisorHelper x y = if x `mod` y == 0 then y
                            else smallestDivisorHelper x (y+1)
